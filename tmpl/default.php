...
<script>
function resizeh(id){
	var heightofdiv = jQuery('#panel_' + id).height();
	jQuery("#image_" + id).css("height",heightofdiv);
}
</script>
...
<?php }else{ ?>
	...
	$caption = preg_replace('/(?:^|\B)#(?![0-9_]+\b)([a-zA-Z0-9_]{1,30})(?:\b|\r)/', '<a class="hashtag" href="https://www<?php echo $modclass; ?>gram.com/explore/tags/$1" target="_blank" rel="nofollow">$0</a>', $caption);
	$caption = preg_replace('/(?:^|\B)@(?![0-9_]+\b)([a-zA-Z0-9_]{1,30})(?:\b|\r)/', '<a class="hashtag" href="https://www<?php echo $modclass; ?>gram.com/$1" target="_blank" rel="nofollow">$0</a>', $caption);
	?>
	<div class="row">
		<div class="col-xs-12">
			<div class="panel panel-default">
				<div class="row no-gutter">
					...
					<div class="col-xs-12 col-md-6 left-col" id="image_<?php echo $item->id; ?>">
						<video controls
					...
					<div class="col-xs-12 col-md-6 left-col">
						<a href="<?php echo $item->permalink; ?>" target="_blank" rel="nofollow">
							<img id="image_<?php echo $item->id; ?>" src="<?php echo $item->media_url; ?>" onload="resizeh(<?php echo "'" . $item->id . "'"; ?>);" />
						</a>
					<?php } ?>
					</div>
					<div id="panel_<?php echo $item->id; ?>" class="col-xs-12 col-md-6">
						<?php if($params->get('show_heading') || $params->get('show_date')){ ?>
						<div class="panel-heading">
							<h3 class="panel-title">
							<?php if($params->get('show_heading')){ ?>
								<span aria-hidden="true" class="fa fa-instagram fa-2x">&nbsp;</span><a href="https://www<?php echo $modclass; ?>gram.com/<?php echo $item->username; ?>" target="_blank" rel="nofollow"><?php echo $item->username; ?></a>
							<?php } ?>
		...
		<?php if(isset($item->thumbnail_url)){ ?><script>resizeh(<?php echo "'" . $item->id . "'"; ?>);</script><?php } ?>
		</div>
		<?php } ?>
